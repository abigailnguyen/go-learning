...
func main() {
	...
	ch <- "cmd.1"
	ch <- "cmd.2" // won't be processed as receiving channel is slow
	close(ch)  // have to close channel if dont want to have fatal error, as receiving on a nil channel will block forever
	<-done
	wg.Wait()
}